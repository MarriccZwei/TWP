...
class StructuralSubmesh():
    ...
    def _sheet_creation(self, inbf:nt.NDArray[np.float64], inbr:nt.NDArray[np.float64], oubf:nt.NDArray[np.float64], oubr:nt.NDArray[np.float64], ny:int, eleType:str, skin:bool=None): raise NotImplementedError
    def _rib_creation(self, istation:int): raise NotImplementedError
    def _railing_creation(self, inb:nt.NDArray[np.float64], oub:nt.NDArray, ny:int, eleArg:list[float], eleType:str): raise NotImplementedError



class UniformStructuralSubmesh(StructuralSubmesh):
    ...
    def _sheet_creation(self, inbf, inbr, oubf, oubr, ny, eleType, skin = None):
        """
...
class StraightStructuralSubmesh(StructuralSubmesh):
    def __init__(self, wing:ElysianWing, HYPERPARAMS:dict[str, float], n:int, rjperc:float):
        self.rjperc = rjperc
        super().__init__(wing, HYPERPARAMS, n)
    ...
    def _rib_creation(self, istation:int):
        ...
        #4) the upper skin
        for xb1, xb2, zb1, zb2 in zip(self.wing.scaffold[istation, :-2:2, 0], self.wing.scaffold[istation, 2::2, 0], self.wing.scaffold[istation, :-2:2, 2], self.wing.scaffold[istation, 2::2, 2]):
            xs, zs = self._ribbing_xzs(xb1, zb1, xb2, zb2, rj)
            for x1, x2, z1, z2 in zip(xs[:-1], xs[1:], zs[:-1], zs[1:]):
                self.eleTypes.append(code)
                self.eleArgs.append([Hsq])
        ...
        #5) the lower skin
        for xb1, xb2, zb1, zb2 in zip(self.wing.scaffold[istation, 1:-2:2, 0], self.wing.scaffold[istation, 3::2, 0], self.wing.scaffold[istation, 1:-2:2, 2], self.wing.scaffold[istation, 3::2, 2]):
            xs, zs = self._ribbing_xzs(xb1, zb1, xb2, zb2, rj)
            for x1, x2, z1, z2 in zip(xs[:-1], xs[1:], zs[:-1], zs[1:]):
                self.eleTypes.append(code)
                self.eleArgs.append([Hsq])
