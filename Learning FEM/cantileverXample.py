...
def test_nat_freq_cantilever(refinement=1, mtypes=range(2)):
    for mtype in mtypes:
        ...
        #@Marricc: an array of all 3-element arrays with xyz
        ...
        #@Marricc: actually starting with the beam element
        p = BeamCProbe()
        data = BeamCData()

        #@Marricc: Stiffness matrices memory reservation
        KC0r = np.zeros(data.KC0_SPARSE_SIZE*num_elements, dtype=INT) #@Marricc: Array to store row positions of sparse values
        KC0c = np.zeros(data.KC0_SPARSE_SIZE*num_elements, dtype=INT) #@Marricc: Array to store column positions of sparse values
        KC0v = np.zeros(data.KC0_SPARSE_SIZE*num_elements, dtype=DOUBLE) #@Marricc: Array to store sparse values

        #@Marricc: Same for mass matrices
        ...
        N = DOF*n #@Marricc: Indeed DOF=6 so 'tis the 6 DOF element
        print('num_DOF', N)

        #@Marricc: computing beam properties, it's a beam along x, with y and z having to be interpreted based on situation
        prop = BeamProp()
        prop.A = A
        prop.E = E
        scf = 5/6. #@Marricc: don't know what it is, but seems a nice way to get G of a material
        ...
        #@Marricc: collapsing the vector step from CompModelling?
        ncoords_flatten = ncoords.flatten()

        beams = []
        init_k_KC0 = 0
        init_k_M = 0
        #@Marricc: element creation loop
        for n1, n2 in zip(n1s, n2s):
            ...
            beam.n1 = n1
            beam.n2 = n2
            #@Marricc: positions in the stiffness matrix
            beam.c1 = DOF*pos1
            beam.c2 = DOF*pos2
